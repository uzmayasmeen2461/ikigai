...
import { CalendarDays, Copy, Download, ImageIcon, Link2, Megaphone, Sparkles, Trash2, Upload } from "lucide-react";
...
function fileToPost(file, index) {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
            const title = cleanTitle(file.name, index);
            resolve({
                id: `${Date.now()}-${index}-${file.name}`,
                fileName: file.name,
                imageUrl: reader.result,
                title,
                caption: `${title}\n\nMake your business easier to discover online with ORVA. Upload once, plan better, and keep your digital presence moving.`,
...
function buildPostText(post) {
    return [
        ...
        "",
        "#ORVA #LocalBusiness #DigitalGrowth #SmallBusiness",
    ].filter(Boolean).join("\n");
}
...
function downloadCsv(posts) {
    const headers = ["file_name", "title", "caption", "cta", "link", "scheduled_for"];
...
export function MarketingContentFlow() {
    ...
    const handleFiles = async (event) => {
        ...
        setMessage({ type: "success", text: `${nextPosts.length} marketing image${nextPosts.length === 1 ? "" : "s"} added. No price needed for this flow.` });
        event.target.value = "";
    };
    ...
    const generateCaption = (post) => {
        updatePost(post.id, {
            caption: `${post.title}\n\nYour customers are already online. ORVA helps your business show up with clear posts, better planning, and a simple digital presence.`,
            cta: post.cta || "Message us to grow your business online",
        });
        setMessage({ type: "success", text: "Caption generated for this marketing image." });
    };
                                ...
                                <article key={post.id} className="interactive-tile grid gap-5 rounded-2xl border border-[var(--border)] bg-white p-4 lg:grid-cols-[220px_1fr]">
                                    <div>
                                        ...
                                        <div className="mt-3 flex items-center justify-between gap-2 text-xs text-[var(--muted)]">
                                            ...
                                            <button type="button" className="text-red-600" onClick={() => setPosts((current) => current.filter((item) => item.id !== post.id))}>
                                    ...
                                    <div className="grid gap-3">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <button type="button" className="btn-primary" onClick={() => generateCaption(post)}>
