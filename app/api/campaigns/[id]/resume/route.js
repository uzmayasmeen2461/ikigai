...
export async function POST(request, context) {
    ...
    try {
        ...
        const nextStatus = campaign.status === "paused" ? "active" : campaign.status;
        const { data, error } = await supabase.from("campaigns").update({ status: nextStatus, updated_at: nowISTISOString() }).eq("id", campaign.id).select("*").single();
        if (error) throw error;
        return NextResponse.json({ campaign: data, message: "Campaign resumed." });
    } catch (error) {
        if (isCampaignSchemaError(error)) return NextResponse.json({ error: campaignsSetupError() }, { status: 503 });
        return NextResponse.json({ error: error.message || "Could not resume campaign." }, { status: 500 });
    }
}
