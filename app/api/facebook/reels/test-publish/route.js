...
import { publishProductReelToFacebookPage } from "../../../../lib/facebookPage";
...
export async function POST(request) {
    ...
    if (!hasSupabaseServiceRoleKey()) return NextResponse.json({ error: "SUPABASE_SERVICE_ROLE_KEY is required before publishing Facebook Reels." }, { status: 503 });
    ...
    if (!product.reel_video_url) return NextResponse.json({ error: "Upload or create a reel video before publishing to Facebook." }, { status: 400 });
    ...
    try {
        ...
        return NextResponse.json({ error: mediaError.message || "Could not prepare reel video for Facebook." }, { status: 500 });
    }
    ...
    if (!mockMode && connection?.status !== "connected") {
        return NextResponse.json({ error: "Connect your Facebook Page before publishing a reel." }, { status: 400 });
    }

    try {
        const result = await publishProductReelToFacebookPage({ product: publishProduct, connection, mockMode, caption });
